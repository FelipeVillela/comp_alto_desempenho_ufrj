...
// Função run reescrita de forma otimizada
int run(double *A, double *b, double *x, double *xtmp)
{
  int itr;
  int row, col;
  ...
  double sqdiff;
  double *ptrtmp;
  double *inverse_diag = malloc(N * sizeof(double));  // Array para armazenar os inversos dos elementos diagonais

  // Pré-calcular inversos dos elementos diagonais
  for (row = 0; row < N; row++)
  {
    inverse_diag[row] = 1.0 / A[row + row*N];
  }

  // Loop até convergência ou atingir o número máximo de iterações
  itr = 0;
  do
  {
    // Executar iteração de Jacobi
    for (row = 0; row < N; row++)
    {
      dot = 0.0;
      for (col = 0; col < N; col++)
      ...
      dot -= A[row + row*N] * x[row];  // Subtrair elemento diagonal
      xtmp[row] = (b[row] - dot) * inverse_diag[row];  // Usar inverso do elemento diagonal pré-calculado
    }
  ...
  free(inverse_diag);  // Liberar memória alocada para o array de inversos
